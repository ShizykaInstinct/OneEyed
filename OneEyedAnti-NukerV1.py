...
# Команда для добавления в белый список (текстовая)
@bot.command()
@commands.has_permissions(administrator=True)
async def whitelist(ctx, user: discord.User):
    global WHITELIST
    WHITELIST.add(user.id)
    save_config()  # Сохраняем конфигурацию после изменения
    await ctx.send(f"Пользователь {user.mention} добавлен в белый список.")

# Команда для удаления из белого списка (текстовая)
@bot.command()
@commands.has_permissions(administrator=True)
...
async def cleanup_commands(interaction: discord.Interaction, command_name: str = "nuke_config"):
    ...
    try:
        ...
        # Тестирование команд на работоспособность
        ...
        for cmd in target_commands:
            ...
            # Попытка "вызова" команды для проверки
            try:
        ...
        # Если рабочая команда была удалена случайно, пересинхронизируем
...
# Регистрация команды cleanup_commands
